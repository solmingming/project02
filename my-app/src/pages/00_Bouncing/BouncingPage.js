...
const BouncingPage = ({ isSettingsOpen, closeSettings }) => {
  ...
  const handleSettingsChange = (newSettings) => setAppSettings(prev => ({ ...prev, ...newSettings }));
  const handleTextChange = (newText) => setAppSettings(prev => ({ ...prev, text: newText }));
  const handleColorChange = (top, bottom) => setAppSettings(prev => ({ ...prev, topColor: top, bottomColor: bottom }));
  ...
  const draw = (p5) => {
    ...
    for (let pt of points) {
      ...
      if (draggingPoint) {
        ...
        const d0 = p5.dist(pt.originalX, pt.originalY, draggingPoint.originalX, draggingPoint.originalY);
    ...
    if (!draggingPoint && selectedPoint && !showSelected) {
      const allRestored = points.every(pt => pt === selectedPoint || (Math.abs(pt.x - pt.originalX) < restoreThreshold && Math.abs(pt.y - pt.originalY) < restoreThreshold));
